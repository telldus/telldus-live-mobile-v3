 ...
 *
 * @providesModule AddFenceArrivingView
 */
...
import { ActionSectionHeader, DeviceRow, EventRow, JobRow, BrownHeader } from 'Geofence_SubViews';
import { setFenceArrivingActions } from 'Actions_Geofence';
import { getUserProfile } from '../../Reducers/User';

class AddFenceArrivingView extends View {

    constructor(props) {
        super(props);
        var initialValue = JSON.parse(JSON.stringify(this.props.fence));
        this.state = {
            devices: initialValue.arriving ? initialValue.arriving.devices : {},
            events: initialValue.arriving ? initialValue.arriving.events : {},
            schedules: initialValue.arriving ? initialValue.arriving.schedules : {},
            showDevices: false,
            showEvents: false,
            showJobs: false
        };
    ...
    toLeaving() {
        this.props.setFenceArrivingActions({
            devices: this.state.devices,
            events: this.state.events,
            schedules: this.state.schedules
        });
        this.props.navigation.navigate('AddFenceLeaving');
    }
    ...
    render() {
        return (
            <View>
                ...
                <ScrollView>
                    <BrownHeader
                        title={'2. Arriving actions'}
                        description={'Select actions for when you arrive'}
                    />
                    <View
                        style={styles.body}
                    >
...
function mapDispatchToProps(dispatch) {
    return {
        setFenceArrivingActions: (actions) => dispatch(setFenceArrivingActions(actions)),
        dispatch,
    };
}


module.exports = connect(mapStateToProps, mapDispatchToProps)(AddFenceArrivingView);
